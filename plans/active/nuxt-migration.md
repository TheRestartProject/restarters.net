...
| A7 | CORS rewrite: tighten existing wildcard config/cors.php to explicit origins (env CLIENT_ORIGINS); delete AddCorsHeaders | ⬜ | supports_credentials stays false; same-origin Blade islands unaffected (verified by critique) |
| A8 | Nuxt 4 scaffold in `client/`: nuxt.config (ssr:false, runtimeConfig), bootstrap-vue-next + @bootstrap-vue-next/nuxt (css:false), pinia+persist, @nuxtjs/i18n (lazy JSON), vitest config, eslint | ⬜ | npm, Node 22; pin exact versions from registry AT SCAFFOLD TIME (verify each exists — do not trust remembered versions); smoke vitest must pass |
