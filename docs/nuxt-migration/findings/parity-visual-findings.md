...
### Profile photo control — deliberate divergence (decided)

develop: `<label>Profile picture:</label>` + `<input type="file">` + a
CHANGE MY PHOTO submit (user/profile/profile.blade.php:138-145).
Ours: the same label, but the shared TusImageUpload (Uppy + resumable tus).

**This is a decided divergence, not an open gap.** Every image upload in this
client - profile photo, group image, event photos, network logo, device
photos - goes through Uppy/tus, and that consistency is the point. Owner
decision, 2026-07-21.

`POST /api/v2/users/me/photo` therefore accepts an `upload_key` only. A
multipart branch was briefly added and has been removed again, so there is
one upload path rather than two to keep in step.
