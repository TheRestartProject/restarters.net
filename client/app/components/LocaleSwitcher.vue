...
<template>
  <div class="language-bar" :class="{ show: open }" data-testid="locale-switcher">
    <div class="d-none" data-testid="locale-current">{{ current }}</div>
...
// Ported from resources/sass/_languages.scss's .language-bar (develop's
// footer language dropdown) - Bootstrap's real .btn-outline-secondary/
// .dropdown-toggle/.dropdown-menu/.dropdown-item classes provide the base
// look (globally imported, see assets/css/restarters.scss), these rules
// only carry the legacy overrides on top. media-breakpoint-up(lg) has no
// mixin available in a component-scoped stylesheet, so it's inlined as the
// bootstrap default lg breakpoint (992px).
.language-bar {
  position: relative;
  z-index: 1;
  background: #fff;
