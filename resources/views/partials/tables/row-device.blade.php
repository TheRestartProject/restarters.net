...
@if( ( Auth::check() && ( FixometerHelper::hasRole(Auth::user(), 'Administrator') || FixometerHelper::userHasEditPartyPermission($device->event, Auth::user()->id) ) ) || ( is_object($is_attending) && $is_attending->status == 1 ) )
<tr class="collapse table-row-details" id="{{ $add ? 'row-add-device' : ('row-' . $device->iddevices) }}">
    <td colspan="11">
        <form id="{{ $add ? 'data-add' : ('data-' . $device->iddevices) }}" class="{{ $add ? 'add-device' : 'edit-device' }}" data-device="{{ $device->iddevices }}" method="post" enctype="multipart/form-data">
