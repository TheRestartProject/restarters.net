...
<tr>
    ...
    @elseif( $event->hasFinished() )
      ...
      @if ( $event->checkForMissingData()['devices_count'] != 0  )
        ...
        <td class="d-none d-sm-table-cell cell-figure">{{{ number_format(round($stats['waste_powered']), 0) }}}<small>kg<small></td>
        <td class="d-none d-sm-table-cell cell-figure">{{{ number_format(round($stats['co2_powered']), 0) }}}<small>kg<small></td>
