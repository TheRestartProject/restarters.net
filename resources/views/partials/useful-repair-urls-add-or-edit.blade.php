    ...
    @foreach( $urls as $url )
        <div class="mb-2 device-select-row">
            <div class="input-group save-url" data-device_id="{{{ $device->iddevices }}}" data-id="{{{ $url->id }}}">
                <input @if( !Auth::check() ) disabled @endif name="url" value="{{{ $url->url }}}" type="url" class="form-control mr-1" placeholder="@lang('devices.useful_repair_urls_helper')" aria-label="@lang('devices.useful_repair_urls_explanation')">
    ...
    @if( Auth::check() && ( FixometerHelper::hasRole(Auth::user(), 'Administrator') || FixometerHelper::userHasEditPartyPermission($device->event, Auth::user()->id) ) )
        <div class="mb-2 device-select-row">
            <div class="input-group add-url" data-device_id="{{{ $device->iddevices }}}">
                <input type="url" name="url" class="form-control w-100" placeholder="@lang('devices.repair_source')" aria-label="@lang('devices.useful_repair_urls_explanation')">
...
{{--                <button class="btn btn-link" type="button"><span class="text-black">+</span></button>--}}
            </div>
        </div>
