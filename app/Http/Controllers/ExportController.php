...
class ExportController extends Controller
{
    ...
    public function devices(Request $request)
    {
        ...
        if ( ! empty($date_from)) {
            ...
            $from = strtotime($d_from->format('Y-m-d').' 00:00:00');
        }

        if ( ! empty($date_to)) {
            $d_to = \DateTime::createFromFormat('Y-m-d', $date_to);
            $to = strtotime($d_to->format('Y-m-d').' 23:59:59');
        }

        if ( ! empty($date_from) && ! empty($date_to)) {
            $all_devices = $all_devices->whereBetween('event_date', [$from, $to]);
        } elseif ( ! empty($date_from)) {
            $all_devices = $all_devices->where('event_date', '>=', $from);
        } elseif ( ! empty($date_to)) {
            $to = $d_to->format('Y-m-d').' 23:59:59';
            $all_devices = $all_devices->where('event_date', '<=', $to);
        }
